...
export default function MedTrackerScreen({ navigation, realm = null, trackerService = medTrackerService }) {
  ...
  const saveScheduleEntry = () => {
    ...
    const scheduledTime = isIntervalSchedule ? '00:00' : normalizeTimeInput(scheduleDraft.scheduledTime);
  ...
  const saveInlineScheduleEntry = (indexToUpdate, updatedFields) => {
    ...
    const scheduledTime = isIntervalSchedule ? '00:00' : normalizeTimeInput(updatedFields.scheduledTime);
    if (!scheduledTime) {
